	...
	var applyWidgetStyles = function(self) {
		...
		if (self.props.text) {
			if (self.hasLabel) {
				...
				if (labelElement) {
					labelElement.innerHTML = PythonIDE.sanitize(Sk.ffi.remapToJs(self.props.text).replace(/ /g, '\u2000'));
				}
			} else {
                console.log("No hasLabel");
				var vimg = "";
...
// Label ---
	s.Label = new Sk.misceval.buildClass(s, function($gbl, $loc) {
		var getHtml = function(self) {
            ...
            v = v.replace(/ /g, ".");//'\u2000');
            console.log("GHTML Label=",v);
			var html = '<div id="tkinter_' + self.id + '" class="tk_charsized" style="display:flex;align-items:center;justify-content:center;">' + v + '</div>';
			return html;
		}
