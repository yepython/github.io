/*
 * JS FX effects generator
 * RIFFWAVE.js v0.03 - Audio encoder for HTML5 <audio> elements.
 * Copyleft 2011 by Pedro Ladaria <pedro.ladaria at Gmail dot com>
 * SFXR to HTML5 by Eric Fredricksen. Maintained by Chris McCormick.
*/
var $builtinmodule = function(name) {
    var mod = {};
    var  fx_presets={
            "jump":"11111Ht16RMFGHsvamep1Wg16xkghappmMP7wh1kG3cjyLJJv7jWLst7YwcZmMKw8bTxbujzWndmgEYzR4PKk3pLJ9FEjg2DABKoe8kRwcvc6T49E3tba4G7",
            "explosion":"7BMHBGDWv7PkpKSQxaGMTWWvGmmYvsFsJ6KPCNp2NLqMmukaj6tZZTiWfuSxeS8iEXQpNEjkZkxcLRz7gKSBaZCVyotJffd9k8E5ipeaHQ8HZyZawUPBQ5LJj",
            "powerup":"111117udoSxPpSV3DUFajTpucivhdK3VkCfQDdQpJkXv5FA55R8EFySPCu4b1Ag5ZHj2bTst7x9ptKeqCy1t1ACvwc7nsgb32jmC6ntMTB8dXfRCr3r7xW2T",
            "shoot":"57uBnWgsmPw5RoX9yraQxNjvus3HbytwPAfQwdoK2LF3C42GPbkKG4yBXa8aXUHtgkM3ig7czunyuSWngYPY757dGk4Eyctr767aFWYMGc2Cb45ro34ECM9rP",
            "laser":"34T6PkvzKzu5y5Vpspv5HSDHAsLBdQt9b9ZmSR5jd4WDQBRsAdoiHT9gQYm5Etq44gh3hEq1n9ZfEzbdYcCHbUfD11sK86A7YFKtqHTHYUaCU3vU7HKJwzE4P",
            "coin-up":"34T6Pkk9EQRxkv9UdhPTGJTVJVPmQfFYMDBi9TS9dyBuMpTFQ4vYfeDsaMrGzipCaBHEXru7Bwc8KnXGyZumX7mhkiAPXXSqGUpf8vXyz8huiJ53MpRbnGCej",
            "pickup-coin":"34T6Pm2UvJ8y8DD6xFhDNrtoreJYbGJjjyAdTdFMRQfbkyUPKrDagKovMPdNL62XF2FL3dKREyNXkzBAhuSoJCAD5hBG7L2PWWNG1C9GJLFrkUpoXwxD2k4Vu",
            "drr":"5EoyNVSymuxD8s7HP1ixqdaCn5uVGEgwQ3kJBR7bSoApFQzm7E4zZPW2EcXm3jmNdTtTPeDuvwjY8z4exqaXz3NGBHRKBx3igYfBBMRBxDALhBSvzkF6VE2Pv"
            }
    function sound_fx(preset) {
            var a = sfxr.toAudio(fx[preset]);
            a.play(); 
    }                
    /*
     * RIFFWAVE.js v0.03 - Audio encoder for HTML5 <audio> elements.
...
//------------------------
    ...
    function make_builtin(func) {
        return new Sk.builtin.func(func);
    }

    mod.sound_fx = make_builtin(function(preset) {
        Sk.builtin.pyCheckArgsLen("sound_fx", arguments.length, 1, 1);
        const preset_str = Sk.ffi.remapToJs(preset);
        if (preset_str in fx_presets) {
            sfxr.play(fx_presets[preset_str]);
        } else {
            throw new Sk.builtin.ValueError("Unknown preset: " + preset_str);
        }
        return Sk.builtin.none.none$;
    });

    mod.custom_fx = make_builtin(function(preset) {
        Sk.builtin.pyCheckArgsLen("custom_fx", arguments.length, 1, 1);
        const preset_str = Sk.ffi.remapToJs(preset);
        sfxr.play(preset_str);
        return Sk.builtin.none.none$;
    });

    mod.rnd_fx = make_builtin(function(preset) {
        Sk.builtin.pyCheckArgsLen("rnd_fx", arguments.length, 1, 1);
        const preset_str = Sk.ffi.remapToJs(preset);
        const sound = sfxr.generate(preset_str);
        sfxr.play(sound);
        return Sk.builtin.none.none$;
    });
    
    mod.test_fx = make_builtin(function(preset) {
        Sk.builtin.pyCheckArgsLen("test_fx", arguments.length, 1, 1);
        const preset_str = Sk.ffi.remapToJs(preset);
        const sound = sfxr.generate(preset_str);
       
        sfxr.play(sound);
        
        let code =sfxr.b58encode(sound);
        return Sk.ffi.remapToPy(code);
    });
    mod.volume_fx = make_builtin(function(v) {
        Sk.builtin.pyCheckArgsLen("volume_fx", arguments.length, 1, 1);
        let vol = Sk.ffi.remapToJs(v);
        if (typeof vol !== "number" || vol < 0 || vol > 100) {
            throw new Sk.builtin.ValueError("The volume should be a number between 0 and 100.");
        }
